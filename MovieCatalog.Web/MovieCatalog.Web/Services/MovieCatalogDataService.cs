...
public class MovieCatalogDataService : IMovieCatalogDataService
{
    ...
    public async Task<Dictionary<Genre, int>> GetGenresWithTitleCountsAsync()
    {
        //Dictionary<Genre, int> dict;
        //var listkey = await ctx.Genres.OrderBy(x => x.Name).ToListAsync();
        //TitleFilter titleFilter = TitleFilter.Empty;
        //foreach (var item in listkey)
        //{
        //    titleFilter.Genres = new List<string> { item.Name };
        //    int howmany = item.TitleGenres.Count();
        //}
        //dict = list.ToDictionary(x => x, x => x.Id);
        List<Genre> genres = await ctx.Genres.ToListAsync();
        Dictionary<Genre, int> dict = genres.OrderBy(x => x.Name).ToDictionary(x => x, x => ctx.TitleGenres.Where(y => y.GenreId.Equals(x.Id)).Count());
        return dict;
    }
    ...
    public async Task<IEnumerable<Genre>> GetGenresAsync()
    {
        var list = await ctx.Genres.OrderBy(x => x.Name).ToListAsync();
        return list;
    }
